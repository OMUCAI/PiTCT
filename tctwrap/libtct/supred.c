...
// #include "curses.h"
	...
	INT_S *tmpu_stack, *cross_stack, index_cross_stack, index_tmpu_stack, *update_flag;
	INT_S s_tmpu_stack;
	INT_S stack_flag = 0;
	...
	//Reduction, Selfloop_Node,Refinement are called in Supreduce and Supreduce2: Strong Control Consistency
	//Reduction1, Selfloop_Node1,Refinement1 are called in Supreduce1: Weak Control Consistency
	//Reduction2, Selfloop_Node2,Refinement2 are called in Supreduce3: New Control Consistency(Dx = Dx')

	FILE *out;

	INT_OS Txt_DES(INT_S);
...
#if defined(_x64_)
	INT_OS Get_DES(INT_S *tran_number,INT_S *num_states,INT_OS index, char *name)
	{
		FILE *in;
		INT_S base_point;
		INT_S mark, s, yy,init;
		INT_T num, z;
		char ch;
		char header[8];
		INT_OS read_endian;
		long block_type, block_size;
		long address; INT_S tmp;
		...
		if(bFormat){
			// read x64 DES
			fread(num_states, sizeof(INT_S), 1, in);
			...
			while(mark!=-1L)
			{
				...
				if(index==0)
				{
					c_marked_states[base_point]=(INT_S)mark;
				}
				if(index==1)
				{
					p_marked_states[base_point]=(INT_S)mark;
				}
				base_point +=1;
			}
			if(index==0)
			...
			*tran_number=0;
			...
			while (s != -1L) {
				...
				//printw("%d\n", num);
				*tran_number += (INT_S)num;
				index_1 += 1;
				for (z=0; z < num; z++) {
					fread(&tran, sizeof(tran_node), 1, in);
			...
			*tran_number = *tran_number+counter;
			...
			if(index==0)
			{
				controller =(INT_S *) MALLOC((3*(*tran_number)+1)*sizeof(INT_S)); /*.....*/
				if (controller == NULL)
				{
					free(c_marked_states); c_marked_states = NULL;
			...
			if(s==-1L)
			{
				...
				for(index_1=0;index_1<*num_states;index_1++)
				{
					if(index==0)
					{
						controller[3*base_point]= index_1;
						controller[3*base_point+1]=1000+index_1;
						controller[3*base_point+2]=index_1;
						base_point += 1;
					}
					if(index==1)
					{
						plant[3*base_point]=index_1;
						plant[3*base_point+1]=5000+index_1;
						plant[3*base_point+2]=index_1;
						base_point +=1;
					}
				}
			}
				...
				while(s!=-1L)
				{
					...
					for(z=0;z<num;z++)
					{
						...
						if(index==0)
						{
							...
							controller[3*base_point]= s;
							//tmp=yy;
							controller[3*base_point+2]= tran.data2;//(int) (yy & 0x003FFFFF);
							controller[3*base_point+1]= (INT_S)tran.data1;//(int) ((unsigned long) tmp >> 22);
						}
						if(index==1)
						{
							if(index_1!=s)
							...
							plant[3*base_point]=s;
							//tmp=yy;
							plant[3*base_point+2]= tran.data2;//(int) (yy & 0x003FFFFF);
							plant[3*base_point+1]= (INT_S)tran.data1;//(int) ((unsigned long) tmp >> 22);
						}
						base_point +=1;
					}
				}
			...
			// read Win32 DES
			fread(num_states, sizeof(INT_OS), 1, in);
			//zprintn(*num_states);
				...
				while(s!=0xFFFFFFFF)
				{
					...
					for(z=0;z<num;z++)
					{
						...
						if(index==0)
						{
							...
							controller[3*base_point]= s;
							tmp=yy;
							controller[3*base_point+2]= (yy & 0x003FFFFF);
							controller[3*base_point+1]= (INT_T) ((unsigned long) tmp >> 22);
						}
						if(index==1)
						{
							if(index_1!=s)
							...
							plant[3*base_point]=s;
							tmp=yy;
							plant[3*base_point+2]= (yy & 0x003FFFFF);
							plant[3*base_point+1]= (INT_T) ((unsigned long) tmp >> 22);
						}
						base_point +=1;
					}
				}
		...
		if(index==0)
		{
			if((*num_states >0) &&(*num_states-1>controller[3*(base_point-1)]))
			{
				for(yy = controller[3*(base_point-1)] + 1; yy < *num_states; yy++)
				{
					controller[3*base_point]=yy;
					controller[3*base_point+1]=1000+yy;
					controller[3*base_point+2]=yy;
					base_point +=1;
				}
			}
		...
		if(index==1)
		{
			...
			{
				for(yy=plant[3*(base_point-1)]+1;yy<*num_states;yy++)
				{
					plant[3*base_point]=yy;
					plant[3*base_point+1]=5000+yy;
					plant[3*base_point+2]=yy;
					base_point +=1;
				}
			}
	...
	INT_OS Get_DES(INT_S *tran_number,INT_S *num_states,INT_OS index, char *name)
	{
		...
		if(!bFormat){   // read WIN32 DES
			...
			*tran_number = *tran_number+counter;
			...
			// In litter endian, reserve the first four byte and discard the latter four byte
			fread(num_states, sizeof(INT_S), 1, in);  fread(&yy, sizeof(INT_S), 1, in); 
			...
			*tran_number = *tran_number+counter;
			...
			else
			{
				...
				while(s!=-1L)
				{
					...
					{
						//fread(&tran, sizeof(tran_node), 1, in);
						fread(&yy,sizeof(INT_S),1,in);    fread(&tmp,sizeof(INT_S),1,in); 
						fread(&yy1,sizeof(INT_S),1,in);   fread(&tmp,sizeof(INT_S),1,in);  
						if(index==0)
						{
							if(index_1!=s)
							{
							...
							tmp=yy;
							controller[3*base_point+2]= yy;
							controller[3*base_point+1]= (INT_T)(yy1 & 0xFFFF);
						}
						if(index==1)
						{
							if(index_1!=s)
							...
							tmp=yy;
							plant[3*base_point+2]= yy;
							plant[3*base_point+1]= (INT_T)(yy1 & 0xFFFF);
						}
						base_point +=1;
					}
				}
	...
	void recover_node(struct node* root_node_1, struct node * root_node_2, INT_S state)
	{
		...
		for(i = 0; i < s_list; i ++){	
			...
			while(temp12 != NULL){
				if((*temp1) == NULL){
					*temp1 = (struct equivalent_state_set *) CALLOC(1,sizeof(struct equivalent_state_set));
					if(*temp1 == NULL){
						mem_result = 1;
						return;
					}
			...
			if(list[i] == equiv_state_2){
				...
				for(j = 1; j < state_par[equiv_state_2].numelts; j ++){
					...
					*temp1 = (struct equivalent_state_set *) CALLOC(1,sizeof(struct equivalent_state_set));
					...
					root_node_1[tmp_state].equal_set->state_number = equiv_state_2;
					temp1 = &((*temp1)->next_node);
				}
				(*temp1) = NULL;
			}
	...
	void divide_nodes(struct node* root_node_1, struct node* root_node_2, INT_S state_1, INT_S state_2)
	{
	...
	/*	if(state_1 == 1 && state_2 == 23)
		...
		temp22 = root_node_2[root_node_2[state_1].equal_set->state_number].equal_set;
		while(temp22 != NULL){
			state = temp22->state_number;
			temp22 = temp22->next_node;
		}
		temp22 = root_node_1[root_node_1[state_1].equal_set->state_number].equal_set;
		while(temp22 != NULL){
			state = temp22->state_number;
			temp22 = temp22->next_node;
		}
		temp22 = root_node_2[root_node_2[state_2].equal_set->state_number].equal_set;
		while(temp22 != NULL){
			state = temp22->state_number;
			temp22 = temp22->next_node;
		}
		temp22 = root_node_1[root_node_1[state_2].equal_set->state_number].equal_set;
		while(temp22 != NULL){
			state = temp22->state_number;
			temp22 = temp22->next_node;
		}*/
		...
		while(temp22 != NULL){
			...
			if(temp11->state_number == state){
				...
				while(temp11 != NULL){
					// remove the equiv set of equiv_state_2 in root_node_2 from equiv set of equiv_state_1 in root_node_1
					temp12 = temp11->next_node;
					if(temp12 != NULL){
		...
		free(list);
		memory_detect(tmp_root_node); memory_detect(root_node);
	}
...
//		INT_B merge_flag;
		...
		//if(root_node_1[temp3].marked_in_plant == true || root_node_1[temp4].marked_in_plant == true)
		//	root_node_1[temp_state].marked_in_plant = true;
		//if(root_node_1[temp3].marked_in_controller == true || root_node_1[temp4].marked_in_controller == true)
		//	root_node_1[temp_state].marked_in_plant = true;
		while(*temp1!=NULL){
	...
	{
		INT_S i, j, k, numelts1, numelts2, tmp_state, state_eq, tmp_state_1, tmp_state_2;
//		INT_B ok;

		if(root_node[state_1].equal_set->state_number == root_node[state_2].equal_set->state_number)
			return;
		else if(root_node[state_1].equal_set->state_number < root_node[state_2].equal_set->state_number){
		...
		for(i = 0; i < numelts1; i ++){
			...
			memcpy(state_par[tmp_state].next + numelts1, state_par[state_2].next, numelts2 * sizeof(INT_S));
		...
		for(i = 0; i < numelts2; i ++){
			...
			memcpy(state_par[tmp_state].next + numelts2, state_par[state_1].next, numelts1 * sizeof(INT_S));
	...
	INT_B add_equiv_state(INT_S state, struct equivalent_state_set * base_node, INT_B *state_number, struct equivalent_state_set ** next_node)
	{	
		if(state_number[state] == true)
			return false;
		*next_node = (struct equivalent_state_set *)MALLOC(sizeof(struct equivalent_state_set));
		if(*next_node == NULL){
			mem_result = 1;
			return false;
		}
	...
	INT_B add_temp_state(INT_S state, struct equivalent_state_set * base_node, struct equivalent_state_set ** next_node, INT_S *s_tmp_list, INT_S **tmp_list)
	{	
		INT_B ok;
	...
	INT_OS Selfloop_Node1(INT_S base_state,INT_S tmp_state,INT_S nodel, INT_OS indexx)
	{
		INT_S node_1,node_2,c_flag, flag, r_flag_1,r_flag_2;
						...
						if((tmp_state_1!=node_1 || tmp_state_2!=node_2) && (tmp_state_1!=node_2 || tmp_state_2!=node_1))
						{
							...
							if(c_flag!=8)
							{
								...
								*(tmpu_stack+2*tmpu_point+2)=-1;
								//zprints("Add to wait list:  "); zprintn(tmp_state_1); zprints("-"); zprintn(tmp_state_2); zprints("\n");
							}
						}
						if(c_flag==0)
						{
									...
									if(temp3->event == temp4->event)
									{
										...
										if((root_node[tmp_state_3].equal_set)->state_number != (root_node[tmp_state_4].equal_set)->state_number)
										{
											...
											if(flag==0)
											{
												...
												*(tmpu_stack+2*tmpu_point+2)=-1;
												//zprints("Add to wait list:  "); zprintn(tmp_state_3); zprints("-"); zprintn(tmp_state_4); zprints("\n");
												flag=Selfloop_Node1(tmp_state_3,tmp_state_4,nodel,indexx);
												if(flag==10)
												{
													if(indexx == 1)
													{
	...
	INT_OS Selfloop_Node2(INT_S base_state,INT_S tmp_state,INT_S nodel, INT_OS indexx)
	{
		INT_S node_1,node_2,c_flag, flag, r_flag_1,r_flag_2;
		INT_S tmpu_point,tmpu_point_1,tmpu_point_2,k;
				...
				while(r_flag_2==0)
				{
					...
					if(tmp_state_2!= -1)
					{
						...
						if((tmp_state_1!=node_1 || tmp_state_2!=node_2) && (tmp_state_1!=node_2 || tmp_state_2!=node_1))
						{
							...
							if((temp1 == NULL && temp11 != NULL) || (temp1 != NULL && temp11 == NULL)){
								return 10;
							}else{
								for(k = 1; k < 3; k ++){
									if(k == 2){
	...
	INT_OS Selfloop_Node3(INT_S base_state,INT_S tmp_state,INT_S nodel, INT_OS indexx)
	{
		INT_S node_1,node_2,c_flag, flag, r_flag_1,r_flag_2;
		INT_S tmpu_point,tmpu_point_1,tmpu_point_2;//,k;
				...
				while(r_flag_2==0)
				{
					...
					if(tmp_state_2!= -1)
					{
						...
						if((tmp_state_1!=node_1 || tmp_state_2!=node_2) && (tmp_state_1!=node_2 || tmp_state_2!=node_1))
						{
							...
							if(root_node[tmp_state_1].marked_in_controller!=root_node[tmp_state_2].marked_in_controller)
								return 10;
								...
								while(temp4 != NULL)
								{
									if(temp3->event == temp4->event)
									{
										...
										if((root_node[tmp_state_3].equal_set)->state_number != (root_node[tmp_state_4].equal_set)->state_number)
										{
											...
											if(root_node[tmp_state_3].marked_in_controller!=root_node[tmp_state_4].marked_in_controller)
												return 10;
											/*temp1 = root_node[tmp_state_3].forb_set;
											temp11 = root_node[tmp_state_4].forb_set;
	...
	INT_OS Selfloop_Node5(INT_S base_state,INT_S tmp_state,INT_S nodel, INT_OS indexx)
	{
		INT_S node_1,node_2,c_flag, flag, r_flag_1,r_flag_2;
		INT_S tmpu_point,tmpu_point_1,tmpu_point_2,k;
		struct forbidden_event_set *temp1;
				...
				while(r_flag_2==0)
				{
					...
					if(tmp_state_2!= -1)
					{
						if(tmp_state_1 == tmp_state_2){
							...
							cm_tmp2 = &(track_table[tmp_state_1].next[tmp_state_2 - 1 - tmp_state_1]);
						}else{
							cm_tmp2 = &(track_table[tmp_state_2].next[tmp_state_1 - 1 - tmp_state_2]);
						}
						if(cm_tmp2->flag == 3){
							return 10;
						}
						...
						if((tmp_state_1!=node_1 || tmp_state_2!=node_2) && (tmp_state_1!=node_2 || tmp_state_2!=node_1))
						{
							...
							if(c_flag ==8) {
								cm_tmp1->numelts += 1;
								cm_tmp1->dynindex = (INT_S*)REALLOC(cm_tmp1->dynindex, (cm_tmp1->numelts)*sizeof(INT_S));
									...
									if(temp3->event == temp4->event)
									{
										...
										if((root_node[tmp_state_3].equal_set)->state_number != (root_node[tmp_state_4].equal_set)->state_number)
										{
											...
											if(tmp_state_3 < tmp_state_4){
												...
												cm_tmp3 = &(track_table[tmp_state_3].next[tmp_state_4 - 1 - tmp_state_3]);
											}else{
												//tmp_cur_3 = tmp_state_4;
												//tmp_cur_4 = tmp_state_3 - 1 - tmp_cur_3;
												cm_tmp3 = &(track_table[tmp_state_4].next[tmp_state_3 - 1 - tmp_state_4]);
											}
											...
											{
												cm_tmp2->numelts += 1;
												cm_tmp2->dynindex = (INT_S*)REALLOC(cm_tmp2->dynindex, (cm_tmp2->numelts)*sizeof(INT_S));
											...
											if(flag==0)
											{
												...
												*(tmpu_stack+2*tmpu_point+2)=-1;
												...
												//zprints("Add to wait list:  "); zprintn(tmp_state_3); zprints("-"); zprintn(tmp_state_4); zprints("\n");
												flag=Selfloop_Node5(tmp_state_3,tmp_state_4,nodel,indexx);
	...
	INT_OS Selfloop_Node51(INT_S base_state,INT_S tmp_state,INT_S nodel, INT_OS indexx)
	{
		INT_S node_1,node_2,c_flag, flag, r_flag_1,r_flag_2;
		INT_S tmpu_point,tmpu_point_1,tmpu_point_2,k;
		struct forbidden_event_set *temp1;
										...
										if((root_node[tmp_state_3].equal_set)->state_number != (root_node[tmp_state_4].equal_set)->state_number)
										{
											...
											if(flag==0)
											{
												...
												*(tmpu_stack+2*tmpu_point+2)=-1;
												...
												//zprints("Add to wait list:  "); zprintn(tmp_state_3); zprints("-"); zprintn(tmp_state_4); zprints("\n");
												flag=Selfloop_Node51(tmp_state_3,tmp_state_4,nodel,indexx);
	...
	// Original version
	/*This module is part of the main module Reduction.*/
	INT_OS Selfloop_Node(INT_S base_state,INT_S tmp_state,INT_S nodel, INT_OS indexx)
	{
		INT_S node_1,node_2,c_flag, flag, r_flag_1,r_flag_2;
		INT_S tmpu_point,tmpu_point_1,tmpu_point_2,k;
		struct forbidden_event_set *temp1;
		...
		while(r_flag_1==0)
		{
			...
			if(tmp_state_1!= -1)
			{
				...
				while(r_flag_2==0)
				{
					...
					if(tmp_state_2!= -1)
					{
						//zprints("sub check merge: "); zprintn(tmp_state_1); zprints("-"); zprintn(tmp_state_2); zprints("\n");
						c_flag=0;
						if((tmp_state_1!=node_1 || tmp_state_2!=node_2) && (tmp_state_1!=node_2 || tmp_state_2!=node_1))
						{
							if(indexx == 1)
				...
				for(j=i+1;j<num_states;j++)
				{
					...
					{	
						...
						if(flag != 10)
						{
							...
							*(tmpu_stack+1)=j;
							*(tmpu_stack+2)=-1;
							//zprints("Add to wait list:  "); zprintn(i); zprints("-"); zprintn(j); zprints("\n");
			...
			if((root_node[i].equal_set)->state_number == i)
			{
				for(j=i+1;j<num_states;j++)
				{
					if((root_node[j].equal_set)->state_number==j)
					{
						...
						if(flag!=10)
						{
							if(root_node[i].marked_in_controller!=root_node[j].marked_in_controller)
								flag = 10;
				...
				for(j=i+1;j<num_states;j++)
				{
					...
					{	
						...
						if(flag != 10)
						{
							...
							*(tmpu_stack+1)=j;
							*(tmpu_stack+2)=-1;
							cm_tmp1->flag = 1; // state pair (i, j) has been added to waiting list
							cm_tmp1->index = 0;
							cm_tmp1->minindex = 0;
							//zprints("Add to wait list:  "); zprintn(i); zprints("-"); zprintn(j); zprints("\n");
						...
						while(*(tmpu_stack+2*tmpu_point)!=-1)
						{
							...
							if(cm_tmp2->flag != 2){
								cm_tmp2->flag = 3;
								//if(track_table[state_1].next[state_2-1-state_1].dynindex != NULL){
								//	free(track_table[state_1].next[state_2-1-state_1].dynindex);
								//	track_table[state_1].next[state_2-1-state_1].dynindex = NULL;
								//}
								tmpu_point += 1;
								continue;
							}
			...
			if((root_node[i].equal_set)->state_number == i)
			{
				for(j=i+1;j<num_states;j++)
				{
					if((root_node[j].equal_set)->state_number==j)
					{
						...
						{
							*tmpu_stack = -1;
						}else if(flag == 100){ // used to indicate whether the stack is overflow
							stack_flag = 100;
							return;
						}
						if(flag != 10)
				...
				while(s!=-1L)
				{
					...
					for(z=0;z<num;z++)
					{
						...
						*temp1=(struct forbidden_event_set *) MALLOC(sizeof(struct forbidden_event_set));
						...
						(*temp1)->event= tran.data1;//((unsigned long) yy>>22);
						if(z<num-1)
							temp1=&((*temp1)->next_event);
						else
				...
				while(s!=0xFFFFFFFF)
				{
					...
					for(z=0;z<num;z++)
					{
						...
						*temp1=(struct forbidden_event_set *) MALLOC(sizeof(struct forbidden_event_set));
						...
						(*temp1)->event= (INT_S)((unsigned long) yy>>22);
						if(z<num-1)
							temp1=&((*temp1)->next_event);
						else
							(*temp1)->next_event=NULL;
				...
				while(s!= -1L)
				{
					...
					for(z=0;z<num;z++)
					{
						...
						*temp1=(struct forbidden_event_set *) MALLOC(sizeof(struct forbidden_event_set));
						...
						(*temp1)->event= (INT_T)( yy & 0x0FFFF);// read two bytes
						if(z<num-1)
							temp1=&((*temp1)->next_event);
						else
							(*temp1)->next_event=NULL;
		...
		for(i=0;i<num_states;i++)
		{
			...
			for(relative_point=0;relative_point<*(controller_tree+base_point+14);relative_point++)
			{
				*temp_1 = (struct transitions *) MALLOC(sizeof(struct transitions));
				...
				(**temp_1).event = (INT_T)(*(controller_tree+base_point+15+2*relative_point));
...
#if defined(_x64_)
	INT_OS Txt_DES(INT_S number_of_states)
	{
		long block_type,block_size,cur_pos,temp,init=0;
		INT_S header_len, i;
		char header[100];
		INT_S base_point,trace_point,j,k,mark_state,counter;
		tran_node tran;
		INT_T tran_number;
		...
		fwrite(&number_of_states,sizeof(INT_S),1,out);
		fwrite(&init,sizeof(INT_S),1,out);
		mark_state=0;
		while(*(c_marked_states+mark_state)!=-1)
		{
			i= *(c_marked_states+mark_state);
		...
		for(k=0;k<number_of_states;k++)
		{
			...
			for(j=0;j<tran_number;j++)
			{
				if(simpler_controller[3*trace_point+1]<1000)
				{
					...
					//tran.data2 = simpler_controller[3*trace_point+2];
					//fwrite(&tran,sizeof(tran_node),1,out);
					yy = simpler_controller[3*trace_point+1];
					yy = yy << 22;
					yy = yy | simpler_controller[3*trace_point+2];
	...
	*tmpu_stack = -1;
	...
	for(j=0;j<num_states-1;j++)
	{
		for(k=j+1;k<num_states;k++)
		{
			flag = 0;
			if(merging_table[j][k] != -1) continue;
			if((root_node[j].marked_in_controller!=root_node[k].marked_in_controller))
				flag = 10;
...
//Modefied by ZRY to implement Localization algorithm without consideration of marking information
INT_OS supreduce1(char *name1,
	char *name2,
	char *name3,
	char *name4,
	INT_S* lb,
	...
	INT_S *fnum)
{
	...
	tmpu_stack=(INT_S *) MALLOC((2*num_states*num_states+1)*sizeof(INT_S));
...
// Modified by ZRY to test a more efficient algorithm 2018.01.15 with quadratic complexity
INT_OS supreduce5(char *name1,
	char *name2,
	char *name3,
	char *name4,
	INT_S* lb,
	float *cr)
{
	...
	for(i = 0; i < s_track_table; i ++){
		...
		track_table[i].next = (check_mark *)CALLOC(track_table[i].numelts, sizeof(check_mark));
	...
	track_root_node = (struct track_node *)CALLOC(num_states, sizeof(struct track_node));
	cross_stack=(INT_S *) MALLOC((2*num_states*num_states+1)*sizeof(INT_S));	
	...
	track_table = NULL;
	free(cross_stack); cross_stack = NULL;
...
void clean_selfloop_orig(INT_S s, state_node *t, INT_S s1, state_node *t1)
{
	...
	s_list = s_list1 = 0; list = list1 = NULL;
	stp = 0; tp = NULL;
